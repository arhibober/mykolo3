 ...
 */
function wp_new_user_notification($user_id, $plaintext_pass = '', $first_name = "", $last_name = "", $role = "") {
	$user = get_userdata( $user_id );
	$f = fopen ("test.txt", "w+");
fwrite ($f,  "test");
fclose ($f);
	...
	//global $wpdb;
	//$sqlstr1 = "SELECT meta_value from wp_usermeta where user_id =".$user_id." and meta_key='first_name'";	
		//$first_name = $wpdb->get_results ($sqlstr1, ARRAY_A);
	if (strlen ($first_name) > 0)	{
		$message .= sprintf (__('First Name: %s'), $first_name) . "\r\n";
		//$sqlstr2 = "SELECT meta_value from wp_usermeta where user_id =".$user_id." and meta_key='last_name'";	
		//$last_name = $wpdb->get_results ($sqlstr2, ARRAY_A);
		$message .= sprintf (__('Last Name: %s'), $last_name) . "\r\n";
		//$sqlstr3 = "SELECT meta_value from wp_usermeta where user_id =".$user_id." and meta_key='role'";	
		//$role = $wpdb->get_results ($sqlstr3, ARRAY_A);
	...
	if ( empty($plaintext_pass) )
		return;
	$message = sprintf(__('You have been registered on the site %s:'), $blogname) . "\r\n\r\n";
	if (strlen ($first_name) > 0){
		$message .= sprintf (__('First Name: %s'), $first_name) . "\r\n";
		$message .= sprintf (__('Last Name: %s'), $last_name) . "\r\n";
		$message .= sprintf (__('Role: %s'), $role) . "\r\n";
	...
	$letter_det = file_get_contents('details.txt');
	echo $letter_det."!!!!!!!!";
	$message .= $letter_det;
	$f = fopen ("test_let.txt", "w+");
fwrite ($f,  "letter");
fclose ($f);
	$letter_det1 = file_get_contents('http://localhost/mykolo3/details.txt');
	echo $letter_det1."!!!!!!!!";
	$f1 = fopen ("test_let1.txt", "w+");
fwrite ($f1,  "letter");
fclose ($f1);
	$headers = 'From: mykolo@example.com' . "\r\n" .
    'Reply-To: mykolo@example.com';
	@wp_mail($user->user_email, sprintf(__('[%s] Your username and password'), $blogname), $message);
	mail($user->user_email, sprintf(__('[%s] Your username and password'), $blogname), $message, $headers);
}
endif;
