...
function thirteen_sidebars() {
	register_sidebar( array(
		'name'          => __( 'Sidebar Area 3', 'thirteen' ),
		'id'            => 'sidebar-3',
		'description'   => __( 'Add widgets here to appear in your sidebar.', 'thirteen' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
...
add_action( 'widgets_init', 'thirteen_sidebars' ); 
...
function username_or_email_login() {
    ...
    // Form Label
    if ( document.getElementById('loginform') )        document.getElementById('loginform').childNodes[1].childNodes[1].childNodes[0].nodeValue = '<?php echo esc_js( __( 'Email', 'email-login' ) ); ?>'; 
...
function wp_registr( $before = '<li>', $after = '</li>' ) {
...
$link = $before . '<a href="' . get_option('http://localhost/mykolo3') . '/wp-login.php?action=register">' . __('Register') . '</a>' . $after;
...
$link = $before . '<a href="' . get_option('http://localhost/mykolo3') . '/wp-admin/">' . __('Site Admin') . '</a>' . $after;}



else {global $current_user; $link = $before . '<a href="' . get_option('http://localhost/mykolo3') . '/bbpress/profile.php?id=' . $current_user->ID . '">' . __('Profile') . '</a>' . $after;
